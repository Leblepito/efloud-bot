...
- **`docs/ROADMAP_AI_INTEGRATION.md`** — AI entegrasyon yol haritası. Tüm ajanlar
  (Gemini, Claude, Antigravity) sıradaki işi bu dosyadan seçer, tamamlanan işleri
  `✅` ile işaretler. Faz 0-4 arası önceliklendirilmiş görev listesi.
- **`docs/skill_log.md`** — Skill kullanım ve etkinlik logu. Her skill kullanımı
  sonrası sonuç buraya yazılır. Düşük performanslı skill'ler revize edilir.
- **`docs/prompt_changelog.md`** — Ajan prompt evolution logu. Prompt değişiklikleri
  ve referans kaynakları burada izlenir.
- **`external_repos/`** — Klonlanmış referans repolar (gitignore'd):
  `autoresearch`, `graphify`, `caveman`, `superpowers`, `system_prompts_leaks`.

### 🏛️ Opus Rolü: MİMAR (Architect)

Efloud çoklu-model ekosisteminde **Claude Opus** mimardır. Gemini Flash mühendistir.

- **Tasarım & Spec:** Yeni feature'lar için spec yaz, API akışlarını tasarla,
  risk analizi yap. Spec'i `docs/handoff/` altına bırak.
- **Code Review:** Flash'ın yazdığı kodu incele, güvenlik açıklarını ve
  mimari uyumsuzlukları yakala.
- **Prompt Optimizasyonu:** `.claude/agents/` prompt'larını refine et,
  `system_prompts_leaks` referanslarıyla güçlendir.
- **Strateji Değerlendirme:** Backtest sonuçlarını analiz et, config önerisi sun.
- **İmplementasyon yapma** — kod yazımı Flash'ın işi. Spec yaz, handoff bırak.

Detaylı görev dağılımı: `docs/ROADMAP_AI_INTEGRATION.md` §6 "Model Dispatch Tablosu".
