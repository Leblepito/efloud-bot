...
class PreDeploymentChecker:
    ...
    def _check_safe_orchestrator(self) -> bool:
        """Safe orchestrator entegrasyonunu test et."""
        try:
            # Load test config
            ...
            # Create orchestrator
            orchestrator = SafeOrchestrator(config)

            # Test initialization
            print(f"   OK SafeOrchestrator Created")
            print(f"   OK Risk Calculator: {type(orchestrator.risk_calculator).__name__}")
            print(f"   OK Notification Manager: {type(orchestrator.notification_manager).__name__}")

            # Test risk calculation integration
            test_balance = 1000.0

            position_size = orchestrator.calculate_position_size("BTC/USDT", test_balance)

            print(f"   OK Integrated Risk Calculation: {position_size:.2f} USDT")
    ...
    def _check_phase_scripts(self) -> bool:
        """Phase execution script'lerinin varlığını ve syntax'ını kontrol et."""
        phase_scripts = [
            "scripts/execute_phase1.py",
            "scripts/execute_phase2.py",
            "scripts/execute_phase3.py",
            "scripts/validate_phase_completion.py"
        ]
        ...
        for script in phase_scripts:
            ...
            try:
                ...
                print(f"   OK {script}: Valid syntax")
