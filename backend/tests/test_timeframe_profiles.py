...
Profiles: scalp=5m/1h/12h, mid=15m/1h/4h, long=1h/8h/1w. Chain must be
...
def test_profiles_constant_matches_spec():
    assert PROFILES["scalp"] == ("5m", "1h", "12h")
    assert PROFILES["mid"] == ("15m", "1h", "4h")
    assert PROFILES["long"] == ("1h", "8h", "1w")
...
def test_profile_takes_precedence_over_explicit():
    ...
    out = resolve_timeframes(cfg)
    assert (out["entry"], out["mtf"], out["htf"]) == ("5m", "1h", "12h")
