...
def test_async_review_releases_in_flight_slot_on_submit_failure(tmp_path):
    """Regression: if ctx-build or executor.submit() raises after the in-flight slot
    is claimed, the trade_id MUST be released from _in_flight_reviews. Otherwise the
    trade stays marked in-flight forever and never gets an advisory review on a later
    cycle (the finally-discard inside task() never runs because task() was never submitted)."""
    mock_agent_team = MagicMock()
    mock_agent_team.cfg = {"enabled": True, "gating": False}

    journal = TradeJournal(str(tmp_path / "trade_journal_leak.jsonl"))
    config = {
        "timeframes": {"htf": "4h", "mtf": "1h", "entry": "15m"},
        "structure": {"swing_lookback": 4, "ob_sequential": 5, "body_mode": True, "eq_threshold_pct": 0.1, "range_lookback": 50},
        "fibonacci": {"ote_lower": 0.618, "ote_upper": 0.786, "ext_tp2": 1.618},
    ...
    orch = SafeOrchestrator(config=config, state_dir=str(tmp_path), freshness_check=False, trade_journal=journal)
    orch.agent_team = mock_agent_team

    pos = orch.lifecycle.open_position(
        symbol="BTC/USDT", direction="LONG", entry_price=100.0, size=1.0, sl=95.0, tp1=110.0, tp2=120.0,
    )

    # Force executor.submit to raise AFTER the in-flight slot has been claimed.
    with patch.object(orch._review_executor, "submit", side_effect=RuntimeError("executor down")):
        orch._run_agent_review_async(
            pos=pos, confluence=60, htf_bias="BULL", df_htf=None, adx=22.0, rr=2.0, size_notional_pct=1.5,
        )

    # The slot must be released (not leaked) so a later cycle can retry.
    assert pos.id not in orch._in_flight_reviews

    # Proof of practical effect: a subsequent call (executor restored) runs the review.
    mock_agent_team.review_trade.return_value = {"team_verdict": "APPROVE", "score": 0.8, "reasons": []}
    orch._journal_record_entry(pos)
    orch._run_agent_review_async(
        pos=pos, confluence=60, htf_bias="BULL", df_htf=None, adx=22.0, rr=2.0, size_notional_pct=1.5,
    )
    for _ in range(20):
        ...
        if snap and snap.agent_review is not None:
            break
    snap = journal.get(pos.id)
    assert snap is not None and snap.agent_review is not None
