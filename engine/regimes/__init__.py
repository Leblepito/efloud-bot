        ...
        if self.model is not None:
            try:
                ...
                if ml_confidence >= 65 and self.ml_can_override_gate:
                    ...
                    # a gate-open one, nor clear should_tighten_stops. The gate-open
                    # set MUST match can_open_new_position (VOLATILE is open only when
                    # allow_volatile_entries) — otherwise ML could widen e.g.
                    # VOLATILE→REVERSAL when VOLATILE is actually gate-blocked.
                    open_regimes = ("TRENDING", "REVERSAL") + (
                        ("VOLATILE",) if self.allow_volatile_entries else ()
                    )
                    det_open = regime in open_regimes
                    ml_open = ml_regime in open_regimes
