...
def generate_signals(
    ...
    for brk in e_brks:
        ...
        if is_long:
            ...
            # TP1: fiyatın ÜSTÜNDE bir hedef lazım
            risk_tmp = abs(price - sl)
            # HTF'de fiyatın üstündeki ilk önemli seviye (FVG üst sınırı veya swing)
            htf_above_targets = [f.top for f in htf_fvgs
                                   if f.direction == "BULL" and f.top > price]
            htf_above_targets += [s.price for s in htf["swing_highs"]
                                    if s.price > price]
            tp1 = min(htf_above_targets) if htf_above_targets else (price + risk_tmp * 2.0)
        else:
            sl_c = [s for s in e_sh if s.idx < brk.idx]
            sl = sl_c[-1].price if sl_c else price * 1.01
            risk_tmp = abs(price - sl)
            htf_below_targets = [f.bot for f in htf_fvgs
                                   if f.direction == "BEAR" and f.bot < price]
            htf_below_targets += [s.price for s in htf["swing_lows"]
                                    if s.price < price]
            tp1 = max(htf_below_targets) if htf_below_targets else (price - risk_tmp * 2.0)
