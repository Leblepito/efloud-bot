...
def run_prediction(ticker: str, period: str, interval: str, pred_len: str, df_path: str | None = None) -> int:
    env = os.environ.copy()
    env["KRONOS_REPO"] = str(KRONOS_DIR)
    cmd = [str(venv_python()), str(INNER), ticker, period, interval, pred_len]
    if df_path:
        cmd.extend(["--df-path", df_path])
    proc = subprocess.run(
        cmd,
        env=env,
    )
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: run_kronos.py <TICKER> [period] [interval] [pred_len] [--df-path <path>]")
    ...
    if "--df-path" in sys.argv:
        try:
            ...
            # Remove --df-path and its value to clean positionals
    ...
    ensure_kronos_repo()
    ensure_venv()
    return run_prediction(ticker, period, interval, pred_len, df_path)
