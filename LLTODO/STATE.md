...
2026-06-18  T-025 DONE ✅       @hermes  P-002 Faz A M3 (Manus REST client + task templates). `backend/social/manus_client.py` 460 satır — `ManusClient` + `requests.Session` transport + create_task/get_task_status/wait_for_completion + retry/backoff (MAX_RETRIES=3, BACKOFF_BASE_SEC=1.5) + key maskeleme + log truncation (1024 char) + fail-safe (flag OFF / key yok → `ManusDisabled`); `backend/social/templates/manus_{x_thread,youtube_short,weekly_snapshot}.json` (3 şablon, her birinde TR+EN compliance token zorunlu); `docs/runbooks/manus-setup.md` (operatör için API key edinme + VPS env + smoke test); `config.yaml` notifications.manus.enabled=false şema eklendi (canlı config'lere DOKUNULMADI); 41/41 unit test PASS (hermetic — network YOK, retry/timeout/key maskeleme/compliance validate dahil). **Bugfix (aynı branch):** `urllib`→`requests` transport migration — Python `urllib` TLS fingerprint (JA3) Hetzner IPv6'da AWS WAF tarafından 403 dönüyordu (curl 200 alırken). `requests` (urllib3) farklı JA3 → bypass. Regression-proof: 41/41 unit test PASS, `_SESSION` mock'lu. **Real API verified:** `GET /v2/task.list` → HTTP 200, `request_id=92657253a0e0e55663e14652ca394ad5` (side-effectsiz, sadece listeleme), hesap aktif (api key adı `hermes`, mevcut task `Bnk8FCrVYgZ6Kavx3eA332` "Dry-Run Connector Readiness Re-Check and Reporting"). **Env wired:** `MANUS_API_KEY` `.env.production`'a eklendi (chmod 600, `MANUS_API_ENABLED=false` default-OFF). Container recreate gerektirmez — bot autostart=0 manuel kontrol altında. **Canlı aktivasyon operatör**: `MANUS_API_ENABLED=true` + `docker compose up -d` recreate.
