...
2026-06-18  T-021 AS-CODE_DONE @hermes  (görev BACKLOG kalır — operatör-gated, T-020 emsali) feat/p003-wr-t021-uptimerobot-as-code branch'inde (master 4e25901 bazlı): UptimeRobot monitör + status page AS-CODE oldu. ops/uptimerobot/monitors.yaml (keyword monitör, "status":"ok" yoksa DOWN → HALT'ı "operational" göstermez, healthz-contract.md kontratı) + scripts/setup_uptimerobot.py (idempotent create-or-update, friendly_name eşleştirme, UPTIMEROBOT_API_KEY env'den, --dry-run key gerektirmez) + tests/scripts/test_setup_uptimerobot.py (20 test, API mock'lu ağ-yok, ruff temiz) + docs/runbooks/uptimerobot-as-code.md (operatör adımları). Diğer hazır patch'lere (t018/t019/wr-t021 status.html) DOKUNULMADI. Transfer: format-patch + sha256. KALAN = operatör canlı-aktivasyon: hesap + API key + Telegram contact ID (opsiyonel) + script çalıştır + public status URL → STATE'e ekle → LIVE/DONE.
```
