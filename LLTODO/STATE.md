...
2026-06-18  T-025 IN_PROGRESS @hermes P-002 Faz A M3 (Manus REST client + task templates). `backend/social/manus_client.py` 410 satır — ManusClient + create_task/get_task_status/wait_for_completion + retry/backoff (MAX_RETRIES=3, BACKOFF_BASE_SEC=1.5) + key maskeleme + log truncation (1024 char) + fail-safe (flag OFF / key yok → ManusDisabled); `backend/social/templates/manus_{x_thread,youtube_short,weekly_snapshot}.json` (3 şablon, her birinde TR+EN compliance token zorunlu); `docs/runbooks/manus-setup.md` (operatör için API key edinme + VPS env + smoke test); `config.yaml` notifications.manus.enabled=false şema eklendi (canlı config'lere DOKUNULMADI); 41/41 unit test PASS (hermetic — network YOK, retry/timeout/key maskeleme/compliance validate dahil). P-002 plan §2 M3 + §4 S5 secrets korundu. **Canlı aktivasyon operatör**: Manus hesabı aç + API key üret + VPS `/etc/efloud-backup.env`'e MANUS_API_KEY ekle + smoke test çalıştır (runbook §2).
